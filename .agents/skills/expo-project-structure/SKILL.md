...
Agent instructions live at the repo root — `AGENTS.md`, with project skills under `.agents/skills`
and Playbooks under `.agents/playbooks/`. Other config and assets stay outside `src/`: `app.json` /
`app.config.ts`, `eas.json`, `package.json`, `assets/`, and `scripts/`.
