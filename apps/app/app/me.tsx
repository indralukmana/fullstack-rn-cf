import { useGetHealth, useGetMe } from "@rn-cf/api-client";
...
export default function MeScreen() {
  ...
  return (
    <View className="flex-1 gap-4 bg-slate-50 px-6 py-8">
      ...
      <Text className="text-sm text-slate-500">API: {env.apiUrl}</Text>
      ...
      <View className="rounded-xl border border-slate-200 bg-white p-4">
        <Text className="mb-2 text-sm font-semibold uppercase text-slate-500">/health</Text>
        <Text className="text-slate-700">
          {healthQuery.isLoading
            ? "Loading…"
            : healthQuery.data?.data
              ? `${healthQuery.data.data.status} · ${healthQuery.data.data.service}`
              : "Unavailable"}
        </Text>
      </View>
